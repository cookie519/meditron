...
def one_shot_prompt(sample):
    data = dataset.filter(lambda example: example['category'] == sample['category'])
    example = data[randrange(len(data))]
...
### Input Text: '''
        ...
        prompt = '''### Task: 
Extract the exact name or names of symptoms from the input text and output them in a list. only list the exact name from the text, do not explain.
### Definition:
